...
//            switch result {
//            case .success(let feed):
//                self?.parseFeed(feed, completion: completion) // Передаємо completion в parseFeed
//            case .failure(_):
//                completion()
//            }
//        }
...
//                let description = item.description ?? ""
//
//                let imageUrl = extractHighQualityImageUrl(
//                    from: item.media?.mediaContents ?? [],
//                    thumbnail: item.media?.mediaThumbnails ?? [],
...
//    private func parseFeed(_ feed: Feed, completion: @escaping () -> Void) {
//        newsItems = []
//        let dispatchGroup = DispatchGroup() // Група для синхронізації асинхронних завдань
//
//        if let rssFeed = feed.rssFeed {
//            for item in rssFeed.items ?? [] {
//                let title = item.title ?? "No title"
...
//                var imageData: Data? = nil
//
//                // Якщо є imageUrl, завантажуємо зображення асинхронно
//                if !imageUrl.isEmpty, let url = URL(string: imageUrl) {
//                    dispatchGroup.enter() // Вказуємо, що починається асинхронне завантаження
...
//                        }
//                        dispatchGroup.leave() // Вказуємо, що завдання завершене
//                    }
//                }
//
//                // Створюємо об'єкт NewsItem
//                let newsItem = NewsItem(
//                    title: title,
//                    imageUrl: imageUrl,
//                    imageData: imageData,
...
//        }
//
//        // Чекаємо завершення всіх асинхронних завдань
