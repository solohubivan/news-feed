...
class NewsItemsManager {
    ...
    func saveNewsItem(_ item: NewsItem) {
        ...
        do {
            try context.save()
            savedItems.append(item)
            print("Збережено новину: \(item.title)")
            print("Кількість новин у savedItems після збереження: \(savedItems.count)")
        } catch {
            print("Не вдалося зберегти новину: \(error)")
        }
    }
    ...
    func deleteNewsItem(_ item: NewsItem) {
        ...
        do {
            ...
            try context.save()
            savedItems.removeAll { $0.title == item.title && $0.datePublished == item.datePublished }
            print("Видалено новину: \(item.title)")
            print("Кількість новин у savedItems після видалення: \(savedItems.count)")
        } catch {
            print("Не вдалося видалити новину: \(error)")
        }
    }
    ...
    private func loadSavedItems() {
        ...
        do {
            ...
            print("Не вдалося завантажити збережені новини: \(error)")
        }
    }
}
